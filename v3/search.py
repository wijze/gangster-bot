...
class Search:
    ...
    def start_search(self, board):
        ...
        if self.settings.iterative_deepening:
            ...
            search_process = Process(
                target=self.iterative_deepening_search, args=(target_depth)
            )
    ...
    def not_quiet_search(self, alpha, beta, depth=0):
        if depth > self.settings.capture_search_depth:
            return beta  # TODO come up with a better solution
        if self.board.turn:
            evaluation = evaluate_board(self.board)
        else:
            evaluation = -evaluate_board(self.board)
        ...
        if evaluation > beta:
            return beta  # this position won't be reached because it's to good
        if evaluation > alpha:
            alpha = evaluation
